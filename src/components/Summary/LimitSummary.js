  ...
  return (
    <ul className="total-limit-summery">
      ...
      <li>
        ...
        {cashServiceInfo.agree ? (
          <LimitSummaryItem
            limit={cashServiceInfo.limit}
            isApproximate={false}
          />
        ) : (
          <button class="btn-limit">한도보기</button>
        )}
      </li>
      <li>
        <div>장기카드대출(카드론)</div>
        {cardLoanInfo.agree ? (
          <LimitSummaryItem limit={cardLoanInfo.limit} isApproximate={true} />
        ) : (
          <button class="btn-limit">한도보기</button>
        )}
      </li>
